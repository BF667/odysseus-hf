...
Key features:
- **Persistent** — data survives Space restarts and rebuilds
- **Private** — buckets are private by default, accessible only with your token
- **S3-compatible** — familiar `hf://buckets/` URI scheme for programmatic access
- **Region-aware** — choose storage region (`us`, `eu`, etc.) when creating a bucket
- **Free tier** — HF provides generous free storage for personal and org use
...
2. An HF API token with **read** and **write** permissions — create one at [https://huggingface.co/settings/tokens](https://huggingface.co/settings/tokens)
...
In your Space's **Settings** page:

| Type | Name | Value | Required |
|------|------|-------|----------|
| **Secret** | `HF_TOKEN` | Your HF API token (`hf_xxx...`) | Yes |
| **Variable** | `ODYSSEUS_AUTO_BUCKET` | `1` | Recommended |
| **Variable** | `ODYSSEUS_HF_BUCKET` | `username/odysseus-data` | Optional |
| **Variable** | `ODYSSEUS_BUCKET_REGION` | `us` | Optional |

**`HF_TOKEN`** (required) — The Space needs your token to access HF Bucket APIs and sync data. Set this as a **Secret** to keep it private.

**`ODYSSEUS_AUTO_BUCKET`** — Set to `1` and the entrypoint will automatically create a private bucket named `odysseus-data` under your HF username on first boot. This is the easiest option — just set it and forget it.

**`ODYSSEUS_HF_BUCKET`** — If you already have a bucket or want a specific name, set this instead (or in addition to `ODYSSEUS_AUTO_BUCKET`). Format: `username/bucket-name` or `org-name/bucket-name`. The entrypoint will use this bucket directly without creating a new one.

**`ODYSSEUS_BUCKET_REGION`** — Storage region for auto-created buckets. Defaults to `us` if not set.
...
1. **Validates your HF token** — logs a warning if `HF_TOKEN` is missing
2. **Creates or selects a bucket** — if `ODYSSEUS_AUTO_BUCKET=1`, it creates `username/odysseus-data`; if `ODYSSEUS_HF_BUCKET` is set, it uses that bucket directly
3. **Syncs existing data** — if the bucket already has files from a previous run, they are pulled down so you pick up where you left off
4. **Starts ChromaDB** — a local embedded instance on port 8100 (no separate container needed)
...
Auth is **disabled by default** on HF Spaces (`AUTH_ENABLED=false`) since HF Spaces already gate access through your Space's visibility setting (Private = only you). If you want an additional login layer, set `AUTH_ENABLED=true` as a Space variable and configure credentials in Settings.
...
| `/api/buckets/create` | POST | Create a new bucket |
| `/api/buckets/select` | POST | Set active bucket |
| `/api/buckets/clear` | POST | Clear active bucket |
| `/api/buckets/info/{id}` | GET | Bucket metadata |
| `/api/buckets/delete/{id}` | DELETE | Delete a bucket |
| `/api/buckets/upload` | POST | Upload a file |
| `/api/buckets/files` | GET | List files in bucket |
| `/api/buckets/files/delete` | POST | Delete files from bucket |
| `/api/buckets/sync` | POST | Sync a directory to bucket |
| `/api/buckets/sync-uploads` | POST | Sync local uploads to bucket |
| `/api/buckets/token` | POST | Update HF token |
...
**Build fails with "Dockerfile not found"**: Make sure `Dockerfile.spaces` exists at the repo root. HF Spaces auto-detects `Dockerfile` by default. If it picks the wrong one, set the `DOCKERFILE` Space variable to `Dockerfile.spaces`.

**Build fails during `git clone`**: The builder stage needs network access to clone the repo from GitHub. If the build can't reach GitHub, check HF Spaces status — occasionally their build runners have network restrictions. As a fallback, you can push the full repo to your Space instead of relying on the auto-clone (use the standard `Dockerfile` with `COPY . .`).

**"No HF_TOKEN set" warning in logs**: Add `HF_TOKEN` as a **Secret** (not a Variable) in your Space settings. Variables are visible in the UI; secrets are not.

**Bucket creation fails**: Ensure your token has **write** permission for Buckets. Go to [https://huggingface.co/settings/tokens](https://huggingface.co/settings/tokens), edit your token, and enable the "Buckets" permission.

**Space shows "Runtime error"**: Check the Logs tab. The most common cause is ChromaDB failing to start — ensure port 8100 is not already in use inside the container. The entrypoint waits up to 30 seconds for ChromaDB to become healthy.

**Data disappeared after rebuild**: This is expected if no bucket was configured. Set `ODYSSEUS_AUTO_BUCKET=1` or `ODYSSEUS_HF_BUCKET=username/bucket-name` as a Space variable so the entrypoint can sync data from the bucket on startup.

**Want to use the standard Dockerfile instead?** The `Dockerfile.spaces` is specifically tuned for HF Spaces — multi-stage build with auto-clone, port 7860, embedded ChromaDB, no auth, and bucket bootstrap. For local Docker or other cloud hosts, use the standard `Dockerfile` with `docker compose up -d --build` as described in the Docker section above.

**Want to deploy a fork or custom branch?** Edit the `ARG ODSY_REPO` and `ARG ODSY_BRANCH` lines at the top of `Dockerfile.spaces` before pushing it to your Space. See the build args table in Step 3 above.
...
| `HF_TOKEN` | -- | HF API token for Storage Bucket access and model downloads |
