...
class UploadHandler:
    def __init__(self, base_dir: str, upload_dir: str, hf_bucket_storage=None):
        ...
        # HF Bucket cloud storage (optional)
        self._hf_bucket_storage = hf_bucket_storage
    ...
    def save_upload(self, u: UploadFile, client_ip: str, owner: str = None) -> dict:
        ...
        # Upload to HF bucket if configured (async, non-blocking best-effort)
        if self._hf_bucket_storage and self._hf_bucket_storage.is_configured:
            try:
                remote_path = f"uploads/{file_id}"
                self._hf_bucket_storage.upload_file(
                    local_path=file_path,
                    remote_path=remote_path,
                )
                file_metadata["cloud_uri"] = f"hf://buckets/{self._hf_bucket_storage.active_bucket_id}/{remote_path}"
                file_metadata["cloud_synced"] = True
                logger.info(f"File synced to HF bucket: {remote_path}")
            except Exception as e:
                logger.warning(f"Failed to sync file to HF bucket: {e}")
                file_metadata["cloud_synced"] = False

        return file_metadata

    def set_hf_bucket_storage(self, storage) -> None:
        """Set or update the HF bucket storage backend."""
        self._hf_bucket_storage = storage
